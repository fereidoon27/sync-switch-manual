...
# Display expanded transfer options
echo -e "\nTransfer options:"
...
echo "3. Transfer a specific file or folder from main machine to destination VM"
echo "4. Transfer a specific file or folder from source VM to main machine"
read -p "Choose an option (1-4) [1]: " TRANSFER_OPTION
...
if [[ "$TRANSFER_OPTION" == "3" ]]; then
    # Only test destination connection for main machine to VM transfers
    if ! test_ssh "$DEST_SSH" "destination VM ($DEST_USER@$DEST_IP:$DEST_PORT)"; then
        exit 1
    fi
elif [[ "$TRANSFER_OPTION" == "4" ]]; then
    # Only test source connection for VM to main machine transfers
    if ! test_ssh "$SOURCE_SSH" "source VM ($SOURCE_USER@$SOURCE_IP:$SOURCE_PORT)"; then
        exit 1
    fi
else
    # Test both connections for VM to VM transfers
    if ! test_ssh "$SOURCE_SSH" "source VM ($SOURCE_USER@$SOURCE_IP:$SOURCE_PORT)"; then
        exit 1
    fi
...
EOF
    ...
    3) # Main machine to VM transfer
        ...
        if [ -d "$LOCAL_SOURCE_PATH" ]; then
            ...
            # Transfer with rsync
            rsync $RSYNC_OPTS -r -e "ssh -p $DEST_PORT" "$LOCAL_SOURCE_PATH/" "$DEST_USER@$DEST_IP:$CUSTOM_DEST_PATH/" || handle_failure "copy from main machine to destination VM"
        else
            IS_LOCAL_DIR="no"
    ...
    4) # VM to main machine transfer
        ...
        if [ "$IS_DIR" = "yes" ]; then
            ...
            log "Transferring remote directory: $SOURCE_PATH to local directory: $LOCAL_DEST_FULL"
            
            # Create local directory
            mkdir -p "$LOCAL_DEST_PATH"
            
            # Transfer with rsync
            rsync $RSYNC_OPTS -r -e "ssh -p $SOURCE_PORT" "$SOURCE_USER@$SOURCE_IP:$SOURCE_PATH/" "$LOCAL_DEST_FULL/" || handle_failure "copy from source VM to main machine"
        else
            # For single file
            FILE_NAME=$(basename "$SOURCE_PATH")
            LOCAL_DEST_FULL="$LOCAL_DEST_PATH/$FILE_NAME"
